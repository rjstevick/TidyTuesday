...
tuesdata$mobile %>% full_join(tuesdata$landline) %>%
  ...
  # plot the data, use group to define one line per country/code
  ggplot(aes(x = datatype, y = value, color = continent, group = code)) +
  # make a panel for each year
  facet_grid(.~year) +
  # add lines for each country that are transparent, wider, and have a rectangle key in the legend
  ...
  # change the color scheme to the Arches parks palette
  ...
  theme(text = element_text(family = "Andale Mono"), legend.position = "bottom", # change all text font and move the legend to the bottom
        ...
        axis.text.x = element_blank(), plot.caption = element_text(hjust = 0.5, size = 8, color = "#8c816c"), # remove x-axis text and edit the caption (centered and brown)
        plot.title = element_text(size = 24), plot.subtitle = element_markdown(size=9, family = "Avenir-Black", color = "#8c816c"), # make the title bigger and edit the subtitle (font and brown)
