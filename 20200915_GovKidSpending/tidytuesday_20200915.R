...
basemap <- kids %>% 
  ...
  scale_fill_viridis_b(name = "Amount per child in $USD \n(adjusted for inflation)",
                       labels = scales::dollar_format(), breaks = c(0,4,8,12,16,20), limits=c(0,20)) +
  ...
  theme(plot.background = element_rect(fill="black"),plot.margin = unit(c(0.5,0.5,0.5,0.5), "cm"),
        legend.position = c(.15,.1), legend.direction = "horizontal", text = element_text(family="Tahoma", color="white"),
...
animate(plot = plotanimate, nframes = length(unique(kids$year)), 
        fps = 4, end_pause = 8, height = 450, width = 670, res = 100)
