...
brazil_loss %>%
   # put all causes into long form per year
   pivot_longer(commercial_crops:small_scale_clearing) %>%
   # select top 5 causes and assign all others to "Other"
   mutate(nameother = fct_lump_n(name, w = value, n = 5)) %>%
   # remove underscores and capitalize
   mutate(nameother = str_to_sentence(str_replace_all(nameother, "_"," "))) %>%
   # group all Others into one name and add up the values per year
   ...
   scale_x_continuous(breaks = breaks_pretty(), expand = c(0,0)) +
